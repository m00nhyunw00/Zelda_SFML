...
{
    sf::Texture* idleTexture = ResourceManager::GetInstance().GetTexture("Cacto_Idle");

    animation.SetOwnerType("CACTO");

    if (idleTexture == nullptr)
    {
        cerr << "Cacto_Idle texture not found." << endl;
...
void Cacto::UpdateTypeLogic(float deltaTime)
{

}

void Cacto::Attack(Creature* target)
{
    ...
    cout << "Cacto Attack : " << realDamage << endl;
}